...
See [SCENARIO_FRAMEWORK.md](SCENARIO_FRAMEWORK.md) for the active campaign spine. Moon, Mars, Jupiter/Io, Saturn, Uranus, and Neptune are mandatory physical destinations; Earth Orbit remains only an internal origin. The Moon's 20-ore industrial contract activates the EVA-only anomaly, and only the recovered/claimed anomaly unlocks Mars. Mars requires 8 Common Ore. Safe Io artifact recovery and later authored outer-planet claims remain explicit route gates. Orbit, flyby, descent, and landing are outcomes in the unified physical Flight system; no Arrival Ops board resumes between them.
...
- Save version 18 is the only accepted schema. Older or malformed payloads are rejected and preserved until the player explicitly confirms New Campaign; no migration or automatic replacement is allowed. The v18 checkpoint uses its own storage key and cannot recover an older campaign.
