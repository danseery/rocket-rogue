...
std::string physicalFlightControlHint(const PanelRenderContext& context)
{
    ...
    return context.invertFlightY
        ...
        : "R-stick left/right rotates · L-stick: left/right strafe · up thrust · down reverse";
}
...
std::string solarMapBody(const PanelRenderContext& context)
{
    ...
    out << "<div class=\"solar-map\">"
        ...
        << (straylightFound ? solarMapNode(straylightIdentityKnown(state.meta.straylightStage) ? "Straylight" : "The Anomaly", straylightIdentityKnown(state.meta.straylightStage) ? "ARK" : "?", "map-vessel", MapKnowledge::Explored) : std::string {})
        << "</div></div>"
