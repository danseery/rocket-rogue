...
- Frontier ladder: reach Moon, Mars, the Jupiter system/Io, Saturn, Uranus, and Neptune in order before continuing to Khepri Prime and the Rift Belt. Earth Orbit remains only as a hidden version-9 save origin. Moon and Mars mining contracts, the Io artifact, and a Perfect Jupiter Flyby are explicit route gates rather than implicit arrival rewards.
...
Reusable campaign content is authored in `src/core/Content.cpp`: add a `ScenarioDefinition` with stable step IDs, events, actions, and rewards; reference a `MiningSiteDefinition` when a step launches a fixed site; and put unlock keys in `Destination::routeRequirementKeys` when a claimed reward gates travel. Procedural scenarios use a non-default template plus a `ScenarioFactoryDefinition`, then create a deterministic saved instance with `makeProceduralScenarioInstance()`. Run catalog validation after any of these changes. UI code consumes `ScenarioObjectivePresentation` and must not duplicate destination-specific progression rules. The full authoring, cocoon, migration, and no-story-branch contract is in [Scenario Framework](docs/SCENARIO_FRAMEWORK.md).
