...
- Frontier ladder: reach Moon, Mars, the Jupiter system/Io, Saturn, Uranus, and Neptune in order before continuing to Khepri Prime and the Rift Belt. Earth Orbit remains only as a hidden version-9 save origin. Moon and Mars mining contracts, the Io artifact, and a Perfect Jupiter Flyby are explicit route gates rather than implicit arrival rewards. Campaign frontier is separate from physical travel: every solar launch carries an authored source-to-target leg, and a blocked Pass Through can require a playable recovery flight rather than silently relaunching from Earth.
...
Reusable campaign content is authored in `src/core/Content.cpp`: add a `ScenarioDefinition` with stable step IDs, events, actions, and rewards; reference a `MiningSiteDefinition` when a step launches a fixed site; put unlock keys in `Destination::routeRequirementKeys` when a claimed reward gates travel; and add a `RouteLinkDefinition` for each solar source/target leg, calibrated fuel profile, recovery policy, and one-way policy. Procedural scenarios use a non-default template plus a `ScenarioFactoryDefinition`, then create a deterministic saved instance with `makeProceduralScenarioInstance()`. Run catalog validation after any of these changes. UI code consumes `ScenarioObjectivePresentation` and route transit state; it must not duplicate destination-specific progression rules or infer an origin from the destination tier. The full authoring, cocoon, migration, and no-story-branch contract is in [Scenario Framework](docs/SCENARIO_FRAMEWORK.md).
...
> **Playtest save boundary:** save schema version 13 intentionally starts fresh. Playtesters must choose **New Game**; version 12 and older saves remain untouched but cannot be continued and show `Progression update requires a new game.`
