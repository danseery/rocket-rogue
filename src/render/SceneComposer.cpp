...
// The mid-hull sides are at roughly x=368/656 in the 1024px rocket sheet.
// Anchor slightly inside that silhouette; the ship draws over the flame root.
constexpr float kSideThrustNozzleShare = 0.14F;
constexpr float kSideThrustWidthShare = 0.12F;
...
// The Ark sheet includes a large transparent canvas. These are the visible
// lower port-engine outlet coordinates measured from its rendered center.
// Keeping the awakening plume tied to this port prevents it from appearing to
// fire through the hull as the cinematic scales the ship.
constexpr float kArkPortEngineExhaustXShare = 0.490F;
constexpr float kArkPortEngineExhaustYShare = 0.066F;
...
constexpr int kMaxSurfacePushTerrainRows = 32;
constexpr int kMaxSurfacePushTerrainColumns = 256;
constexpr std::size_t kMaxSurfacePushForecastMarkers = 14U;
...
bool departingSurface(const RenderSnapshot& snapshot)
{
    return snapshot.screen == Screen::Flight && !snapshot.launchLandingLocalFrame &&
        snapshot.launchHandoffFrom == static_cast<int>(FlightMode::Landing) &&
        snapshot.launchHandoffProgress < 1.0;
}

float departureTerrainOpacity(const RenderSnapshot& snapshot)
{
    return 1.0F-smootherstep(static_cast<float>(snapshot.launchHandoffProgress /
        flight_landing::departureFadeFraction));
}
    ...
    Vec2 travelAnchor = {})
{
    ...
    if (snapshot.flightCameraOverride) {
        ...
        result.landingBlend = snapshot.flightCameraLandingBlend;
        return result;
    }
    if (snapshot.launchDockingActive) {
        ...
        // Reload fallback starts from centered travel framing. A live entry
        // uses the actual outgoing camera in updateFlightCameraPresentation.
        const Camera2D wide {
    ...
    if (snapshot.systemTravel) {
        ...
        const auto* frameBody = snapshot.systemLocation.frame == CoordinateFrame::Body
            ? systemBody(snapshot.system,snapshot.systemLocation.bodyId) : nullptr;
        ...
        // Preserve the established zoom, but frame free travel around the
        // ship and velocity lead rather than the selected waypoint.
        ...
        result.transfer = {{static_cast<float>(snapshot.launchPositionX),
            static_cast<float>(snapshot.launchPositionY)},travelAnchor,scale,departureRotation};
        result.travelWeight = 1.0F;
        if (frameBody && frameBody->id == "earth" && g.targetId == "moon") {
            ...
            result.travelWeight *= 1.0F-launchBlend;
        }
        ...
        result.travelWeight *= 1.0F-strongestBlend;
        if (snapshot.launchOrbitCaptured) result.travelWeight = 0;
    }
    ...
    result.travelWeight *= 1.0F-result.landingBlend;
...
Vec2 launchCorridorPoint(
    ...
    double courseOffset)
{
    const Vec2 center = routePoint(snapshot, progress);
    const Vec2 tangent = routeTangent(snapshot, progress);
    const Vec2 routeRight {tangent.y, -tangent.x};
    const float normalizedOffset = static_cast<float>(
        courseOffset / std::max(0.01, snapshot.launchCourseLimit));
    constexpr float corridorLostHalfWidth = 0.10F;
    return {
        center.x + routeRight.x * normalizedOffset * corridorLostHalfWidth,
        center.y + routeRight.y * normalizedOffset * corridorLostHalfWidth
    };
}
...
int systemBodyAsset(const SystemBodyDefinition& body)
{
    return body.id == "earth" ? EarthAsset : body.id == "mars" ? MarsAsset :
        body.id == "mercury" ? MercuryAsset : body.id == "venus" ? VenusAsset :
        body.id == "jupiter" ? JupiterAsset : body.id == "saturn" ? SaturnAsset :
        body.id == "uranus" ? UranusAsset : body.id == "neptune" ? NeptuneAsset : MoonAsset;
}
...
Color miningMaterialColor(int material, float integrity, bool revealed, bool hazard, int hazardAffinity, int destinationTier, float light)
{
    ...
    case 2:
        ...
        break;
    case 3:
        color = mars ? Color {0.54F, 0.56F, 0.59F, 1.0F} : (moon ? Color {0.60F, 0.61F, 0.63F, 1.0F} : Color {0.50F, 0.53F, 0.57F, 1.0F});
        break;
    case 4:
        color = mars ? Color {0.72F, 0.48F, 0.16F, 1.0F} : (moon ? Color {0.76F, 0.59F, 0.22F, 1.0F} : Color {0.68F, 0.49F, 0.16F, 1.0F});
        break;
    case 5:
        color = mars ? Color {0.58F, 0.26F, 0.72F, 1.0F} : (moon ? Color {0.45F, 0.36F, 0.70F, 1.0F} : Color {0.46F, 0.28F, 0.70F, 1.0F});
        break;
    case 6:
        color = {0.16F, 0.60F, 0.72F, 1.0F};
        break;
    case 7:
...
Color miningRewardGlowColor(int material)
{
    switch (static_cast<MiningCellMaterial>(material)) {
    case MiningCellMaterial::CommonOre:
        return {0.74F, 0.78F, 0.84F, 1.0F};
    case MiningCellMaterial::RareOre:
        return {1.0F, 0.74F, 0.24F, 1.0F};
    case MiningCellMaterial::ExoticVein:
...
{
    // All flight draw paths consume the same resolved presentation camera.
    ...
    if (inputSnapshot.screen == Screen::Flight && inputSnapshot.launchPhysicalFlight &&
        flightCameraPresentation_.initialized) {
        ...
        blendedSnapshot->flightCameraLandingBlend = flightCameraPresentation_.landingBlend;
    }
...
void SceneComposer::beginFrame(const RenderSnapshot& snapshot)
{
    ...
    const UiSurfaceKind surface = snapshot.titleScreen
    ...
    const bool surfaceActive = snapshot.screen == Screen::Mining ||
        (snapshot.surfaceArrivalPrepared && (snapshot.launchLandingLocalFrame || departingSurface(snapshot)));
    if (surfaceActive && snapshot.miningWidth > 0 && snapshot.miningHeight > 0) {
        ...
        surfaceCamera_.progress = snapshot.screen == Screen::Mining || departingSurface(snapshot)
            ? 1.0F : miningLayoutBlend;
        ...
        if (snapshot.screen == Screen::Mining) {
            if (wasActive) surfaceCamera_.followY +=
                surfaceCamera_.miningLayerSiteTopRow - snapshot.miningLayerSiteTopRow;
...
void SceneComposer::drawMiningMaterialMarker(
    float cx,
    float cy,
    float radius,
    int material,
    ...
    bool worldSpace)
{
    ...
    switch (static_cast<MiningCellMaterial>(material)) {
    case MiningCellMaterial::CommonOre:
        // Common is the neutral, workhorse resource: a compact silver hex.
        drawCircle(cx, cy, safeRadius, color, 6, worldSpace);
        return;
    case MiningCellMaterial::RareOre: {
        // Rare keeps the gold diamond and extends it into a four-point
        // starburst so it remains identifiable without relying on hue.
        drawCircle(cx, cy, safeRadius, color, 4, worldSpace);
        const float ray = safeRadius * 1.32F;
        const Color rayColor {color.r, color.g, color.b, color.a * 0.82F};
        drawLine(cx - ray, cy, cx + ray, cy, rayColor, 1.25F, worldSpace);
        drawLine(cx, cy - ray, cx, cy + ray, rayColor, 1.25F, worldSpace);
        return;
    }
    case MiningCellMaterial::ExoticVein:
        // Exotic uses a triangular crystal silhouette with a bright core.
        drawCircle(cx, cy, safeRadius, color, 3, worldSpace);
        drawCircle(
            cx,
            cy,
            safeRadius * 0.28F,
            {1.0F, 0.88F, 1.0F, color.a * 0.88F},
            12,
            worldSpace);
        return;
    case MiningCellMaterial::ArtifactCache:
    ...
    case MiningCellMaterial::HazardPocket:
    case MiningCellMaterial::Bedrock:
        drawRect(cx, cy, safeRadius * 2.0F, safeRadius * 2.0F, color, worldSpace);
        return;
    }
}
...
void SceneComposer::drawMiningCombatText(float cx, float cy, float unitSize, int amount, float age, bool allied, bool critical, bool rigDamage, int kind)
{
    ...
    if (defeatText) {
        ...
        color = {0.74F, 0.78F, 0.84F, 0.94F};
    } else if (rewardRare) {
        color = {1.0F, 0.74F, 0.24F, 0.96F};
    } else if (rewardExotic) {
        color = {0.95F, 0.28F, 0.78F, 0.96F};
    }
...
void SceneComposer::drawMining(const RenderSnapshot& snapshot, bool arrivalComposite)
{
    ...
    if (arrivalComposite) {
        ...
        if (departingSurface(snapshot)) arrivalOpacity = departureTerrainOpacity(snapshot);
    ...
    const double activeActorY = snapshot.miningAnchorValid
        ? snapshot.miningAnchorY
    ...
    auto nearestScannerDistanceCells = [&](double x, double y) {
        double nearest = std::hypot(x - activeActorX, y - activeActorY);
    ...
    const float actorDamagePressure = std::clamp(
        (1.0F - static_cast<float>(
            snapshot.miningOperatorActive
                ? snapshot.miningOperatorIntegrity
                : snapshot.miningDroneHealth)) * 1.1F,
        0.0F,
        1.0F);
    const float damagePressure = snapshot.miningOperatorActive
        ? actorDamagePressure
    ...
    const bool showUpperSeam = !snapshot.miningCellsAbove.empty() && activeActorY < 8.0;
    const bool showLowerSeam = !snapshot.miningCellsBelow.empty() && activeActorY > snapshot.miningHeight - 8.0;
    bool terrainCacheMatches = !showUpperSeam && !showLowerSeam && miningTerrainCacheValid_
    ...
    if (!terrainCacheMatches) {
        ...
        const auto appendTerrainRect = [&](float cx, float cy, float width, float height, Color color,
            ...
            color.a *= drawOpacity_;
            if (arrivalComposite) {
                // Soften finite site edges without changing physical barriers.
                const float sideCells = std::min(cx-left-miningViewOffsetX,
                    right+miningViewOffsetX-cx)/cellW;
                const float edgeCells = std::min(sideCells, (cy-bottom-miningViewOffsetY)/cellH);
                color.a *= smootherstep(edgeCells/3.0F);
            }
        ...
        for (int y=visibleTop; !arrivalComposite && y<=visibleBottom; ++y) {
        ...
        // The same adjoining geology used by cutting/collision is visible at
        // the seam. Unrevealed resources remain a neutral rock silhouette.
        const auto appendSeam = [&](std::span<const MiningCell> cells, int offset, bool visible) {
            if (!visible) return;
            const int first = std::max(0, static_cast<int>(activeActorY) - offset - 8);
            const int last = std::min(static_cast<int>(cells.size()) / snapshot.miningWidth - 1,
                static_cast<int>(activeActorY) - offset + 8);
            for (int row=first; row<=last; ++row) for (int x=0; x<snapshot.miningWidth; ++x) {
                const auto& cell = cells[static_cast<std::size_t>(row*snapshot.miningWidth+x)];
                if (cell.material == MiningCellMaterial::Empty) continue;
                const int y = row + offset;
                const auto center = cellCenter(x,y);
                const float distance = static_cast<float>(std::hypot(x+.5-activeActorX,y+.5-activeActorY));
                const float light = std::clamp(1.0F-distance/8.0F,0.0F,1.0F);
                if (light <= 0) continue;
                if (cell.revealed && texturedTiles) {
                    const int frame = miningTileFrame(cell.material,cell.hazardAffinity,x,row,
                        snapshot.destinationTier,snapshot.miningGeologySeed);
                    const float v0 = postSolarTiles ? static_cast<float>(snapshot.miningPostSolarGeologyRow)/kPostSolarMiningGeologyRows : 0;
                    const float v1 = postSolarTiles ? static_cast<float>(snapshot.miningPostSolarGeologyRow+1)/kPostSolarMiningGeologyRows : 1;
                    const auto uv = mapSceneAtlasUvRect(tileTexture,static_cast<float>(frame)/kMiningTileFrameCount,
                        v0,static_cast<float>(frame+1)/kMiningTileFrameCount,v1);
                    if (uv.valid && uv.page == tileAtlas.page) {
                        const float brightness = .66F*light;
                        appendTerrainRect(center.x,center.y,cellW*1.002F,cellH*1.002F,
                            {brightness,brightness,brightness,1},uv.u0,uv.v0,uv.u1,uv.v1,true);
                        continue;
                    }
                }
                appendTerrainRect(center.x,center.y,cellW*1.002F,cellH*1.002F,
                    {.14F*light,.15F*light,.18F*light,1});
            }
        };
        appendSeam(snapshot.miningCellsAbove,-snapshot.miningAboveHeight,showUpperSeam);
        appendSeam(snapshot.miningCellsBelow,snapshot.miningHeight,showLowerSeam);
    ...
    const auto drawTerrainFog = [&]() {
        ...
        const auto unknownAt = [&](int x, int y) {
            if (snapshot.miningActiveDepth > 0 && y < 0) return 1.0F;
        ...
        const auto fogVertex = [&](float x, float y) {
            ...
            const float exploredOpacity = arrivalComposite ? std::lerp(landingFog,normalFog,normalViewBlend) : normalFog;
            const float opacity = std::max(exploredOpacity,
                miningUpperBoundaryFog(snapshot.miningActiveDepth, gridY));
        ...
        // The coarse fog mesh feathers below the baseline; this exact cap
        // prevents interpolation, scan pulses, or revealed neighboring tiles
        // from opening holes above it. Actors remain drawn over the veil.
        const float capBottom = std::max(fogBottom, gridPoint(0, 0).y);
        if (snapshot.miningActiveDepth > 0 && fogTop > capBottom)
            drawRect((fogLeft+fogRight)*.5F, (capBottom+fogTop)*.5F,
                fogRight-fogLeft, fogTop-capBottom, {0.014F,0.025F,0.042F,1.0F});
    };
    if (arrivalComposite) {
        drawTerrainFog();
    ...
    // A quiet route-back contour remains above the opaque upper veil. It is
    // terrain-derived, so excavated bends stay readable without revealing ore.
    std::vector<SceneVertex>& passageOutline = scratchVertices(256U);
    forEachMiningUpperPassageEdge(snapshot.miningActiveDepth, snapshot.miningCellsAbove,
        snapshot.miningWidth, snapshot.miningAboveHeight,
        [&](int x0, int y0, int x1, int y1) {
            const auto a = gridPoint(x0,y0), b = gridPoint(x1,y1);
            if (std::max(a.y,b.y) < backdropBottom || std::min(a.y,b.y) > viewportTop ||
                std::max(a.x,b.x) < backdropLeft || std::min(a.x,b.x) > backdropRight) return;
            appendLine(passageOutline,a.x,a.y,b.x,b.y,{.30F,.62F,.66F,.28F});
        });
    submitLines(passageOutline,1.0F);
    ...
    for (std::size_t index = 0; index < snapshot.miningLooseObjects.size(); ++index) {
        ...
        const Vec2 velocity {
            ...
            -static_cast<float>(chunk.velocityY) * cellH
        };
        const float speed = std::hypot(velocity.x, velocity.y);
        if (speed > cellSize * 0.05F) {
            const Vec2 trailDirection = normalize({-velocity.x, -velocity.y});
            drawLine(
                position.x,
                position.y,
                position.x + trailDirection.x * std::min(speed * 0.06F, cellSize * 0.46F),
                position.y + trailDirection.y * std::min(speed * 0.06F, cellSize * 0.46F),
                {chunkColor.r, chunkColor.g, chunkColor.b, 0.30F * pulse},
                1.4F);
        }
        drawRadialGlow(
            position.x,
            position.y,
            radius * 2.3F,
            {chunkColor.r, chunkColor.g, chunkColor.b, 0.07F * pulse},
            12);
        drawMiningMaterialMarker(
            position.x,
            position.y,
            radius,
            static_cast<int>(chunk.material),
    ...
    } else if (previousMiningOperatorActive_ != snapshot.miningOperatorActive) {
        previousMiningOperatorActive_ = snapshot.miningOperatorActive;
    ...
    for (const MiningMiniDroneAgent& shieldDrone : snapshot.miningMiniDrones) {
        ...
        Vec2 shieldAnchor = activeActor;
        bool shieldAnchorValid = snapshot.miningAnchorValid;
        switch (shieldDrone.anchorTarget) {
        case MiningAnchorTarget::Rig:
            shieldAnchorValid = snapshot.miningRigPresent;
            shieldAnchor = drone;
            break;
        case MiningAnchorTarget::Operator:
            shieldAnchorValid = snapshot.miningOperatorPresent;
            shieldAnchor = operatorPosition;
            break;
        case MiningAnchorTarget::ControlledActor:
            break;
        }
        if (!shieldAnchorValid) {
            continue;
        }
        const Vec2 shieldDronePosition = cellCenter(shieldDrone.x, shieldDrone.y);
        Vec2 shieldDirection = normalize({
            shieldDronePosition.x - shieldAnchor.x,
            shieldDronePosition.y - shieldAnchor.y
        });
        ...
        if (charge > 0.001F) {
            ...
            for (int segment = 0; segment < arcSegments; ++segment) {
                ...
                drawLine(
                    shieldAnchor.x + std::cos(startAngle) * arcRadius,
                    shieldAnchor.y + std::sin(startAngle) * arcRadius,
                    shieldAnchor.x + std::cos(endAngle) * arcRadius,
                    shieldAnchor.y + std::sin(endAngle) * arcRadius,
                    shieldColor,
                    1.9F + impact * 1.8F);
            }
            const Vec2 arcCenter {
                shieldAnchor.x + shieldDirection.x * arcRadius,
                shieldAnchor.y + shieldDirection.y * arcRadius
            };
            drawRadialGlow(
                arcCenter.x,
                arcCenter.y,
    ...
    for (std::size_t i = 0; !snapshot.miningExtractionActive && i < snapshot.miningMiniDrones.size(); ++i) {
        ...
        if (role == static_cast<int>(MiniDroneRole::Resource) || role == static_cast<int>(MiniDroneRole::Mining)) {
            ...
            for (int slot = 0; slot < capacity; ++slot) {
                ...
                if (slot < commonChunks) {
                    chunkColor = {0.74F, 0.78F, 0.84F, 0.94F};
        ...
        if (role == static_cast<int>(MiniDroneRole::Resource) && behavior == MiningMiniDroneBehavior::Working) {
            ...
            Color transferColor {0.74F, 0.78F, 0.84F, 0.94F};
    ...
    if (snapshot.miningOperatorPresent && snapshot.miningOperatorActive) {
        const Vec2 operatorVelocity {
            static_cast<float>(snapshot.miningOperatorVelocityX) * cellW,
            -static_cast<float>(snapshot.miningOperatorVelocityY) * cellH
        };
        const float operatorSpeed = std::hypot(operatorVelocity.x, operatorVelocity.y);
        if (operatorSpeed > cellSize * 0.10F) {
            const Vec2 coastDirection = normalize({-operatorVelocity.x, -operatorVelocity.y});
            const float coastLength = std::clamp(
                operatorSpeed * 0.08F,
                operatorSize * 0.12F,
                operatorSize * 0.48F);
            for (float side : {-1.0F, 1.0F}) {
                const Vec2 coastSide {-coastDirection.y, coastDirection.x};
                const Vec2 start {
                    operatorPosition.x + coastSide.x * operatorSize * 0.13F * side,
                    operatorPosition.y + coastSide.y * operatorSize * 0.13F * side
                };
                drawLine(
                    start.x,
                    start.y,
                    start.x + coastDirection.x * coastLength,
                    start.y + coastDirection.y * coastLength,
                    {0.32F, 0.86F, 1.0F, 0.16F},
                    1.2F);
            }
        }
        const Vec2 operatorThrust {
            static_cast<float>(snapshot.miningOperatorThrustX) * cellW,
            -static_cast<float>(snapshot.miningOperatorThrustY) * cellH
        };
        const float operatorThrustMagnitude = std::clamp(
            std::hypot(operatorThrust.x, operatorThrust.y) / std::max(0.0001F, cellSize),
            0.0F,
            1.0F);
        if (operatorThrustMagnitude > 0.04F) {
            const Vec2 plumeDirection = normalize({-operatorThrust.x, -operatorThrust.y});
            const Vec2 suitSide {-operatorAimDirection.y, operatorAimDirection.x};
            const Vec2 jetpackCenter {
                operatorPosition.x - operatorAimDirection.x * operatorSize * 0.23F,
                operatorPosition.y - operatorAimDirection.y * operatorSize * 0.23F
            };
            const float plumeFlicker = 0.84F + 0.16F * std::sin(animationTime * 22.0F);
            for (float side : {-1.0F, 1.0F}) {
                const Vec2 nozzle {
                    jetpackCenter.x + suitSide.x * operatorSize * 0.11F * side,
                    jetpackCenter.y + suitSide.y * operatorSize * 0.11F * side
                };
                const Vec2 tip {
                    nozzle.x + plumeDirection.x * operatorSize * (0.28F + operatorThrustMagnitude * 0.20F) * plumeFlicker,
                    nozzle.y + plumeDirection.y * operatorSize * (0.28F + operatorThrustMagnitude * 0.20F) * plumeFlicker
                };
                const Vec2 plumeSide {-plumeDirection.y, plumeDirection.x};
                const float halfWidth = operatorSize * 0.045F;
                drawTriangle(
                    nozzle.x + plumeSide.x * halfWidth,
                    nozzle.y + plumeSide.y * halfWidth,
                    nozzle.x - plumeSide.x * halfWidth,
                    nozzle.y - plumeSide.y * halfWidth,
                    tip.x,
                    tip.y,
                    {0.16F, 0.72F, 1.0F, 0.48F + operatorThrustMagnitude * 0.36F});
                drawRadialGlow(
                    nozzle.x,
                    nozzle.y,
                    operatorSize * 0.11F,
                    {0.62F, 0.96F, 1.0F, 0.09F + operatorThrustMagnitude * 0.08F},
                    10);
            }
        }
    }
    ...
    if (snapshot.miningRigPresent &&
        !snapshot.miningExtractionActive &&
        textureReady(MiningDroneAsset)) {
        ...
        drawSpriteRotated(
            ...
            -hullDirection.y,
            snapshot.miningRigDisabled
                ? Color {heatTint.r * 0.46F, heatTint.g * 0.48F, heatTint.b * 0.54F, 0.92F}
                : heatTint,
            MiningDroneAsset);
    } else if (snapshot.miningRigPresent && !snapshot.miningExtractionActive) {
    ...
    if (snapshot.miningRigPresent &&
        !snapshot.miningExtractionActive &&
        snapshot.miningRigDisabled) {
        const float disabledPulse = 0.5F + 0.5F * std::sin(animationTime * 6.0F);
        drawLine(
            drone.x - droneSize * 0.24F,
            drone.y - droneSize * 0.24F,
            drone.x + droneSize * 0.24F,
            drone.y + droneSize * 0.24F,
    ...
    if (snapshot.miningRigPresent && !snapshot.miningExtractionActive) {
        ...
        if (toggleProgress > 0.001F && toggleProgress < 0.999F) {
            const float radius = droneSize * 0.61F;
            drawEllipseLine(
                drone.x,
                drone.y,
                radius,
                radius,
                {0.04F, 0.16F, 0.19F, 0.72F},
                42,
                -kPi * 0.5F,
                kPi * 1.5F);
            drawEllipseLine(
                ...
                radius,
                radius,
                {0.30F, 1.0F, 0.76F, 0.92F},
                42,
                -kPi * 0.5F,
                -kPi * 0.5F + kPi * 2.0F * toggleProgress);
        }
        const float togglePulseAge = miningOperatorTogglePulseStartedAt_ < 0.0
            ? 1.0F
            : static_cast<float>(snapshot.animationTime - miningOperatorTogglePulseStartedAt_);
        if (togglePulseAge >= 0.0F && togglePulseAge < 0.32F) {
            const float pulseProgress = togglePulseAge / 0.32F;
            drawEllipseLine(
                drone.x,
                drone.y,
                droneSize * (0.50F + pulseProgress * 0.38F),
                droneSize * (0.50F + pulseProgress * 0.38F),
                {0.38F, 1.0F, 0.78F, (1.0F - pulseProgress) * 0.84F},
                42,
                0.0F,
                kPi * 2.0F);
        }
    ...
    if (!snapshot.miningExtractionActive &&
        snapshot.miningOperatorPresent &&
        textureReady(JetpackCapybaraAsset)) {
        const Color operatorTint = miningHeatSpriteTint(snapshot.miningHeat, snapshot.animationTime);
    ...
    if (!snapshot.miningExtractionActive &&
        ...
        if (firePulse > 0.001F) {
            const Vec2 muzzle {
                operatorPosition.x + operatorAimDirection.x * operatorSize * 0.42F,
                operatorPosition.y + operatorAimDirection.y * operatorSize * 0.42F
            };
            drawRadialGlow(
                muzzle.x,
                muzzle.y,
                operatorSize * (0.16F + firePulse * 0.12F),
                {0.72F, 1.0F, 1.0F, 0.08F + firePulse * 0.13F},
                14);
            drawCircle(
                muzzle.x,
                muzzle.y,
                operatorSize * (0.045F + firePulse * 0.025F),
                {0.90F, 1.0F, 1.0F, 0.72F + firePulse * 0.24F},
                10);
        }
    }
    ...
    if (snapshot.miningExtractionActive) {
        ...
        if (snapshot.miningOperatorPresent && textureReady(JetpackCapybaraAsset)) {
            const float suitEntry = smoothExtraction((extractionProgress - 0.16F) / 0.32F);
            const Vec2 suitPosition {
                operatorPosition.x + (extractionBay.x - operatorPosition.x) * suitEntry,
                operatorPosition.y + (extractionBay.y - operatorPosition.y) * suitEntry
            };
            const float suitSize = operatorSize * (1.0F - suitEntry * 0.42F);
            const float suitAlpha = suitEntry < 0.999F
                ? 1.0F
                : std::max(0.0F, 1.0F - extractionClose);
            const Vec2 extractionDirection = normalize({
                extractionBay.x - operatorPosition.x,
                extractionBay.y - operatorPosition.y
            });
            const Vec2 suitSpriteForward = normalize({
                -extractionDirection.x - extractionDirection.y,
                extractionDirection.x - extractionDirection.y
            });
            if (suitAlpha > 0.001F) {
                drawLine(
                    ...
                    suitPosition.x,
                    suitPosition.y,
                    {0.38F, 0.94F, 1.0F, (1.0F - suitEntry) * 0.24F},
                    1.8F);
                drawSpriteRotated(
                    suitPosition.x,
                    suitPosition.y,
    ...
    if (!snapshot.miningExtractionActive &&
        snapshot.miningOperatorActive &&
        snapshot.miningDrilling) {
        const Vec2 handDrillMuzzle {
            operatorPosition.x + operatorAimDirection.x * operatorSize * 0.42F,
            operatorPosition.y + operatorAimDirection.y * operatorSize * 0.42F
        };
        const float dx = target.x - handDrillMuzzle.x;
        const float dy = target.y - handDrillMuzzle.y;
        const float requestedReach = std::max(0.0F, dx * operatorAimDirection.x + dy * operatorAimDirection.y);
        const float maxReach = cellSize * 1.2F;
        const float reach = std::min(requestedReach, maxReach);
        particleAnchor = {
            handDrillMuzzle.x + operatorAimDirection.x * reach,
            handDrillMuzzle.y + operatorAimDirection.y * reach
        };
        const float drillPulse = 0.70F + 0.30F * std::sin(animationTime * 24.0F);
        drawLine(
            handDrillMuzzle.x,
            handDrillMuzzle.y,
            particleAnchor.x,
            particleAnchor.y,
            {0.18F, 0.78F, 1.0F, 0.28F * drillPulse},
            5.0F);
        drawLine(
            handDrillMuzzle.x,
            handDrillMuzzle.y,
            particleAnchor.x,
            particleAnchor.y,
            {0.82F, 1.0F, 1.0F, 0.88F * drillPulse},
            1.6F);
        drawRadialGlow(
            particleAnchor.x,
            particleAnchor.y,
            cellSize * (0.28F + 0.08F * drillPulse),
            {0.32F, 0.92F, 1.0F, 0.10F + 0.06F * drillPulse},
            12);
    }
...
{
    const auto profile = service_dock::profile(snapshot.launchDockId);
    ...
    if (profile.parallel) {
        // Two hull-mounted shoes rise from the inner wall to the ship's side.
        for (double station : {-.12, .12}) {
            const double tipY = std::lerp(profile.backstop - .025, profile.centerY - service_dock::hullRadius, close);
            const Vec2 mount = point(station, profile.backstop - .05);
            const Vec2 end = point(station, tipY);
            drawLine(mount.x, mount.y, end.x, end.y, {.18F,.24F,.28F,1}, 12);
            drawLine(mount.x, mount.y, end.x, end.y, {.60F,.67F,.70F,1}, 5);
            const Vec2 a = point(station-.045,tipY), b = point(station+.045,tipY);
            drawLine(a.x,a.y,b.x,b.y,{.40F,.48F,.52F,1},6);
            if (snapshot.launchDockSecuring) drawCircle(end.x,end.y,.012F*scale,light,12);
        }
    } else for (const double side : {-1.0, 1.0}) {
        const double base = side * profile.halfWidth;
        const double tip = std::lerp(base, side * service_dock::hullRadius, close);
        const Vec2 mount = point(base + side * .035, profile.centerY);
        const Vec2 end = point(tip, profile.centerY);
        const Vec2 sleeve = point(base - side * .065 * close, profile.centerY);
        ...
        const Vec2 padA = point(tip, profile.centerY - .055);
        const Vec2 padB = point(tip, profile.centerY + .055);
    ...
    if (locked) {
        ...
        for (const double side : {-1.0, 1.0}) {
            const Vec2 marker = point(side * profile.halfWidth, profile.mouth);
    ...
    if (locked) drawPoiLabel(0.0F, .66F, .0033F, snapshot.launchDockName, PoiGuidanceKind::Ship);
}
...
void SceneComposer::drawRoute(const RenderSnapshot& snapshot)
{
    ...
    if (snapshot.screen == Screen::Flight && snapshot.launchPhysicalFlight) {
        ...
        if (snapshot.launchDockingActive) {
            ...
            const Vec2 outward = view.camera.vector(std::cos(snapshot.launchDockHeading), std::sin(snapshot.launchDockHeading));
            const Vec2 right {outward.y, -outward.x};
            const float mouth = static_cast<float>(profile.mouth) * view.camera.scale;
            const float channel = static_cast<float>(profile.halfWidth) * view.camera.scale;
            const float pocket = static_cast<float>(profile.centerY) * view.camera.scale;
            const float pocketHalfWidth = static_cast<float>(profile.targetHalfWidth) * view.camera.scale;
            const float pocketHalfDepth = static_cast<float>(profile.targetHalfDepth) * view.camera.scale;
        ...
        } else if (snapshot.launchLandingLocalFrame && !snapshot.surfaceArrivalPrepared) {
    ...
    if (snapshot.screen == Screen::Flight && snapshot.launchManualControlsEnabled) {
        const auto drawCorridorBoundary = [&](double courseOffset, Color color, float width) {
            ...
            for (float side : {-1.0F, 1.0F}) {
                Vec2 previous = launchCorridorPoint(snapshot, 0.0F, courseOffset * side);
                for (int i = 1; i <= 28; ++i) {
                    const float t = static_cast<float>(i) / 28.0F;
                    const Vec2 next = launchCorridorPoint(snapshot, t, courseOffset * side);
...
void SceneComposer::updateFlightCameraPresentation(const RenderSnapshot& snapshot) noexcept
{
    ...
    if (!snapshot.systemTravel) { state.initialized = false; state.travel = {}; return; }
    // Simulation time, not wall-clock presentation time: menus, disconnects,
    // and required story pauses must not accumulate camera acquisition time.
    const double now = snapshot.animationTime;
    const double dt = state.lastTime >= 0 ? std::clamp(now-state.lastTime,0.0,.1) : 0.0;
    if (state.lastTime >= 0 && now < state.lastTime) {
        state.initialized = false; state.travel = {}; state.dockSourceValid = false;
        state.transitionStart = state.abortSeconds = -1.0;
    }
    state.lastTime = now;
    const auto* frameBody = snapshot.systemLocation.frame == CoordinateFrame::Body
        ? systemBody(solarSystemDefinition(),snapshot.systemLocation.bodyId) : nullptr;
    SystemVector origin = frameBody ? frameBody->position : SystemVector{};
    double units = 1;
    if (snapshot.launchDockingActive) {
        const auto* earth = systemBody(snapshot.system,snapshot.launchDockId);
        if (earth) origin = systemDockPosition(*earth);
        units = service_dock::localUnitsPerSystemUnit;
    }
    const auto decode = [](const std::array<float,6>& c) {
        return Camera2D{{c[0],c[1]},{c[2],c[3]},c[4],c[5]};
    };
    const auto encode = [](const Camera2D& c) {
        return std::array<float,6>{c.focus.x,c.focus.y,c.anchor.x,c.anchor.y,c.scale,c.rotation};
    };
    const auto rebase = [&](const std::array<float,6>& c) {
        auto result = decode(c);
        result.focus = {static_cast<float>((c[0]/state.units+state.originX-origin.x)*units),
            static_cast<float>((c[1]/state.units+state.originY-origin.y)*units)};
        result.scale *= static_cast<float>(state.units/units);
        return result;
    };
    const auto layout = resolveUiViewportLayout(std::max(1,viewport_.logicalWidth),
        std::max(1,viewport_.logicalHeight),uiSurfaceKindForScreen(Screen::Flight));
    const double width = std::max(1,layout.sceneRect.width), height = std::max(1,layout.sceneRect.height);
    const double pixelUnit = std::max(1.0,std::min(width,height)*.5*kSceneViewportPadding);
    auto view = physicalFlightCamera(snapshot,state.approachBlend);
    const bool eligible = !snapshot.launchDockingActive && !snapshot.launchLandingLocalFrame &&
        !snapshot.surfaceArrivalPrepared && !snapshot.launchOrbitCaptured && view.travelWeight > .999F;
    const double vx = snapshot.launchVelocityX + (frameBody ? frameBody->velocity.x : 0);
    const double vy = snapshot.launchVelocityY + (frameBody ? frameBody->velocity.y : 0);
    const auto direction = view.transfer.vector(vx,vy);
    state.travel.update(dt,direction.x,direction.y,std::hypot(vx,vy)*flight_geometry::velocityToMetersPerSecond,
        eligible,width,height,travel_camera::shipRenderSize*pixelUnit*.71);
    const Vec2 anchor{static_cast<float>(state.travel.offset.x*width/pixelUnit),
        static_cast<float>(state.travel.offset.y*height/pixelUnit)};
    view = physicalFlightCamera(snapshot,state.approachBlend,anchor);
    auto camera = view.camera;
    const std::string frame = std::to_string(static_cast<int>(snapshot.systemLocation.frame)) + snapshot.systemLocation.bodyId;
    if (state.initialized && !state.docking && snapshot.launchDockingActive) {
        state.dockSource = encode(rebase(state.current));
        state.dockSourceValid = true;
        state.transitionStart = -1;
    } else if (state.initialized && state.docking && !snapshot.launchDockingActive) {
        state.from = encode(rebase(state.current));
        state.abortSeconds = 0;
        state.dockSourceValid = false;
    }
    if (snapshot.launchDockingActive && state.dockSourceValid) {
        auto closeSnapshot = snapshot;
        closeSnapshot.launchDockHandoffProgress = 1;
        const auto close = physicalFlightCamera(closeSnapshot,state.approachBlend).camera;
        camera = blendCameraTrackingPoint(decode(state.dockSource),close,
            smootherstep(static_cast<float>(snapshot.launchDockHandoffProgress)),{0,0});
    } else if (state.abortSeconds >= 0) {
        state.abortSeconds += dt;
        camera = blendCamera(decode(state.from),camera,
            smootherstep(static_cast<float>(state.abortSeconds/flight_landing::handoffSeconds)));
        if (state.abortSeconds >= flight_landing::handoffSeconds) state.abortSeconds = -1;
    } else if (eligible && state.initialized && frame == state.frameId &&
    ...
    if (eligible && state.transitionStart >= 0.0) {
        const float progress = static_cast<float>(std::clamp((now - state.transitionStart) / 1.25, 0.0, 1.0));
        // A waypoint may change fit/zoom, but must not steal positional lead.
        camera.scale = std::exp(std::lerp(std::log(std::max(.0001F,state.from[4])),
            std::log(std::max(.0001F,camera.scale)),smootherstep(progress)));
        if (progress >= 1.0F) state.transitionStart = -1.0;
    }
    state.current = encode(camera);
    state.originX = origin.x; state.originY = origin.y; state.units = units;
    state.docking = snapshot.launchDockingActive;
    state.landingBlend = view.landingBlend;
    state.targetId = snapshot.flightGuidance.targetId;
    state.frameId = frame;
...
{
    if (snapshot.systemTravel && snapshot.system.id == "solar" && !snapshot.launchLandingLocalFrame) {
        const auto* body = snapshot.systemLocation.frame == CoordinateFrame::Body
            ? systemBody(snapshot.system,snapshot.systemLocation.bodyId) : nullptr;
        const SystemVector offset = body ? body->position : SystemVector{};
        const auto view = physicalFlightCamera(snapshot,flightCameraPresentation_.approachBlend);
        int index = 0;
        for (const auto& asteroid : solarAsteroidBelt()) {
            const auto p = view.camera.point(asteroid.position.x-offset.x,asteroid.position.y-offset.y);
            const float size = static_cast<float>(asteroid.radius*2.0)*view.camera.scale;
            ++index;
            if (std::abs(p.x)>sceneAspect_+size || std::abs(p.y)>1.2F+size) continue;
            const float angle = static_cast<float>(index*2.31+snapshot.animationTime*.16);
            if (textureReady(AsteroidAsset))
                drawSpriteRotated(p.x,p.y,size,size,std::sin(angle),std::cos(angle),{1,1,1,1},AsteroidAsset);
        ...
        if (approachingSolarAsteroidBelt({snapshot.launchPositionX+offset.x,snapshot.launchPositionY+offset.y},
                {snapshot.launchVelocityX+(body ? body->velocity.x : 0.0),
                 snapshot.launchVelocityY+(body ? body->velocity.y : 0.0)}))
...
void SceneComposer::drawRocket(const RenderSnapshot& snapshot)
{
    ...
    if (surfaceCamera_.active || (snapshot.surfaceArrivalPrepared && snapshot.manualAscentCameraProgress<1.0)) {
        ...
        double horizontal = snapshot.launchLandingHorizontalPosition;
        double altitude = snapshot.launchLandingAltitude;
        if (departingSurface(snapshot)) {
            const double nx=std::cos(snapshot.launchLandingBasisAngle), ny=std::sin(snapshot.launchLandingBasisAngle);
            const double dx=snapshot.launchPositionX-snapshot.launchHandoffX;
            const double dy=snapshot.launchPositionY-snapshot.launchHandoffY;
            horizontal+=(ny*dx-nx*dy)*flight_geometry::velocityToMetersPerSecond;
            altitude+=(nx*dx+ny*dy)*flight_geometry::velocityToMetersPerSecond;
        }
        route=view.gridPoint(snapshot.launchLandingPadX+horizontal/4.0+0.5,
            snapshot.launchLandingPadY-altitude/4.0,offset);
        route.y+=ascentSpriteSize*0.455F;
        if (departingSurface(snapshot)) {
            const float blend=static_cast<float>(departureCameraProgress(snapshot.launchHandoffProgress));
            route={std::lerp(route.x,orbitRoute.x,blend),std::lerp(route.y,orbitRoute.y,blend)};
        }
    }
    const Vec2 right {forward.y, -forward.x};
    ...
    float scale = snapshot.launchDockingActive
        ? std::lerp(travel_camera::shipRenderSize,
    ...
    if (ascentSpriteSize>0.0F) scale=departingSurface(snapshot)
        ? std::lerp(ascentSpriteSize,0.17F*1.35F,
            static_cast<float>(departureCameraProgress(snapshot.launchHandoffProgress))) : ascentSpriteSize;
    ...
    if (snapshot.screen == Screen::Flight
        ...
        const float rocketSize = (departingSurface(snapshot)
        ...
        const int frame = static_cast<int>(snapshot.animationTime * 18.0) % 6;
        // UV top is the flame root. Cropping the empty leading rows lets the
        // root stay on the hull while the small plume animates outwards.
        submitInstance(
            {nozzle.x + exhaustSide.x*halfLength, nozzle.y + exhaustSide.y*halfLength,
                -exhaustSide.y*halfWidth, exhaustSide.x*halfWidth,
                -exhaustSide.x*halfLength, -exhaustSide.y*halfLength,
                {1.0F,1.0F,1.0F,.98F},
                static_cast<float>(frame)/6.0F, kThrustSheetLeadingTransparentShare,
    ...
    if (snapshot.preflightActive) {
        ...
        if (textureReady(RocketOpenAsset)) {
            const float size = (departingSurface(snapshot)
                ? std::lerp(1.0F,0.86F,static_cast<float>(departureCameraProgress(snapshot.launchHandoffProgress)))
                : surfaceCamera_.active ? std::lerp(0.86F, 1.0F, surfaceCamera_.progress) : 0.86F) * scale;
            texturedQuad(RocketOpenAsset, size, size, {1.0F, 1.0F, 1.0F, 1.0F});
        }
...
void SceneComposer::drawStraylightSequence(const RenderSnapshot& snapshot)
{
    ...
    // The sun is established while the Ark powers up and remains in the same
    // shot through boarding. On departure it simply recedes with the Ark,
    ...
    if (stage == Stage::Boarding || stage == Stage::EvacuationBriefing) {
        // A civilian evacuation should read as a loose flotilla, not a six-ship
        // parade. Most craft rise from below the frame while the remainder arc
        // in through the lower-left, all converging on the Ark's bay.
        constexpr int evacuationCraftCount = 32;
        for (int i=0;i<evacuationCraftCount;++i) {
            const int lane = (i * 5) % 11;
            const int wave = i / 11;
            const bool fromBelow = i % 3 != 0;
            const float spread = static_cast<float>(lane) / 10.0F;
            const float sx = fromBelow
                ? -.96F + spread * 1.62F
                : -1.18F - .05F * static_cast<float>(wave);
            const float sy = fromBelow
                ? -1.08F - .07F * static_cast<float>(wave) - .04F * (i % 2)
                : -.92F + spread * .48F;
            const float launchDelay = .20F * static_cast<float>(i % 11) + .18F * static_cast<float>(wave);
            const float progress=stage == Stage::Boarding
                ? std::clamp((t-launchDelay)/4.8F,0.0F,1.0F)
                : .06F + .025F * static_cast<float>(i % 6);
            if (progress>=1) continue;
            const float arc = (fromBelow ? 1.0F : -1.0F) * std::sin(progress*kPi) *
                (.025F + .008F * static_cast<float>(i % 4));
            const float craftX=sx+(bayX-sx)*progress+arc;
            const float craftY=sy+(bayY-sy)*progress;
            const float craftSize=.020F+.004F*static_cast<float>(i%4);
            drawSprite(craftX,craftY,craftSize,craftSize,{.8F,.95F,1,1},RocketClosedAsset);
        }
    }
    ...
    if ((stage == Stage::Awakening && t>9) || stage == Stage::Departing) {
        drawSpriteRotated(x-size*kArkPortEngineExhaustXShare,
    ...
    const std::string_view title = stage == Stage::Reveal && t<8 ? "CONTACT RESOLVED" :
        ...
        stage == Stage::Departing ? "LEAVING THE SOLAR SYSTEM" : stage >= Stage::Arrived ? "AARU VALE" :
        straylightIdentityKnown(stage) ? "STRAYLIGHT" : "THE ANOMALY";
...
void SceneComposer::drawBackdrop(const RenderSnapshot& snapshot)
{
    ...
    if (snapshot.launchDockingActive) {
        ...
        if (const auto* earth = systemBody(snapshot.system, snapshot.launchDockId)) {
            ...
            if (earth->kind == SystemBodyKind::Station) {
                const auto hullUp = view.camera.vector(0,1);
                const float light = snapshot.straylightStage >= StraylightStage::Awakening ? 1.0F : .46F;
                drawSpriteRotated(earthPoint.x,earthPoint.y,earthRadius*2.4F,earthRadius*2.4F,
                    hullUp.x,hullUp.y,{light,light,light,1.0F-handoff},
                    arkDamaged(snapshot.arkCondition) ? ArkDamagedAsset : ArkOperationalAsset);
            } else drawSprite(earthPoint.x, earthPoint.y, earthRadius * 2.0F, earthRadius * 2.0F,
                {1.0F, 1.0F, 1.0F, 1.0F - handoff}, systemBodyAsset(*earth));
        }
        ...
        if (profile.parallel) {
            ...
            drawSpriteRotated(center.x + outward.x * static_cast<float>(profile.artOffsetY) * artScale,
                ...
                static_cast<float>(profile.artHeight) * view.camera.scale,
                outward.x, outward.y, {1,1,1,handoff}, static_cast<int>(TextureId::StraylightDock)-1);
        } else drawSpriteRotated(center.x, center.y, dockWorldSize * view.camera.scale, dockWorldSize * view.camera.scale,
    ...
    if (snapshot.screen == Screen::Flight && snapshot.launchPhysicalFlight) {
        ...
        const float destinationAlpha = departingSurface(snapshot)
            ? 1.0F-departureTerrainOpacity(snapshot)
            : 1.0F - smootherstep((view.landingBlend - 0.72F) / 0.26F);
        ...
        // Physical identity, not the parent planet's shared environment art.
        const auto* activeBody = snapshot.systemTravel
            ? systemBody(snapshot.system,snapshot.systemLocation.bodyId) : nullptr;
        if (!activeBody && snapshot.systemTravel)
            activeBody = systemBody(solarSystemDefinition(),snapshot.systemLocation.bodyId);
        if (activeBody)
            destinationSize = static_cast<float>(systemBodyDisplayRadius(*activeBody))*2.0F*view.camera.scale;
        if (snapshot.surfaceArrivalPrepared && !departingSurface(snapshot)) {
        ...
        const int destinationAsset = activeBody ? systemBodyAsset(*activeBody) : destinationBodyAsset(snapshot);
        ...
        if (snapshot.orbitalOverlay > 0.001 ||
            ...
            for (const auto& layer : snapshot.orbitalSurveyLayers) {
                const float visibility = (layer.artifact && snapshot.orbitalZoneSurveyed ? 1.0F :
                    std::clamp(reveal - layer.depth, 0.0F, 1.0F)) * alpha;
            ...
            drawOrbitalArtifactSignal(snapshot,c.x,c.y,radius,destinationAlpha,approachBlend);
...
void SceneComposer::finalizePacket()
{
    ...
    packet_.droppedFrameInstances = droppedFrameInstances_;
}
