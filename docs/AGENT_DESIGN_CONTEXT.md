...
- Save version 10 retains version 9's scenario/mining state and adds persisted Launch Upgrade ranks plus four lesson milestones. Migration keeps `earth_orbit` at index 0 internally, sends a truly pristine v9 campaign to the zero-credit Moon tutorial, maps progressed Earth saves conservatively, grants location-appropriate minimum ranks to Moon-or-later saves, and converts retired proving-card ownership before pruning old IDs.
