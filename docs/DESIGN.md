...
3. If landing, enter the same physical Mining world with a visible Rig tank, independent Rig/Suit oxygen, a 24-mass Rig cap, and contextual scanner/drill/EVA actions.
...
Flight outcomes retain exact Research Data, credits, route effects, and one-shot rewards without opening an Arrival board. Campaign-critical beats remain explicit claims: deliver 20 lunar Common Ore, recover the EVA-only lunar anomaly to unlock Mars, deliver 8 Mars Common Ore, commission the Hazard Support Drone Mk I on Io, recover the protected Io artifact, then continue the authored outer-planet objectives. Repeated orbit loops or artifacts cannot grind a gate.
...
Campaign and expedition progression flows through core systems: scenario gates and authored rewards in `ResearchSystem`/`ScenarioSystem`, physical Mining rules in `MiningSystem`, finite Rig fuel in `RigFuelSystem`, and deterministic cargo ownership in `PayloadTransfer`. Panels and app transitions consume those helpers instead of duplicating rules.
...
Save version 18 is a strict fresh-start boundary. The loader accepts exactly version 18. Older, future, and malformed campaign payloads are rejected and preserved; they expose no Continue action and are overwritten only after the player explicitly confirms New Campaign. Preferences remain intact. Stable hub states write a separate v18 checkpoint; incompatible older checkpoints are never offered for recovery.
...
- `src/core/SaveData.*` persists only the current version-18 campaign and physical Flight/Mining state. Every non-v18 payload is rejected before any field can restore.
...
The campaign save format is versioned and line-based. Version 18 is the current and only accepted schema. It persists the unified Flight run, planetary expedition, physical Mining world, fuel and oxygen tanks, loose objects, crew identity/status, scenarios, cargo, and current progression. Non-v18 campaigns are not migrated or automatically deleted. A confirmed New Campaign explicitly replaces them with a clean v18 save. Validated stable hubs additionally write a separate v18 checkpoint. `SaveSchema.h` remains authoritative for current field names and defaults.
