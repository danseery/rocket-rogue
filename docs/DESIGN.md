...
Arrival choices retain saved introductions and show exact Research Data, credits, route effects, launch boosts, and descent hazard before commitment. Pass Through cannot bypass authored Moon, Mars, Io, Jupiter, or Saturn objectives. At Uranus, the authored Neptune Vector requires exactly two Flight Data keys: the first permanent Uranus artifact supplies one and a Good or Perfect Orbit supplies the second. Landings and repeated artifacts do not grind this gate. Orbit begins on the Perfect solution, so maintaining the gold band for the full loop earns the top grade. Campaign-critical beats use mandatory, non-dismissible briefings and explicit claims: deliver 30 lunar Common Ore and install Prospector Mk I/Slot 1; deliver 40 Mars Common Ore and fabricate empty Slot 2; commission the Hazard Support Drone Mk I on Io, cool and mine one four-segment thermal seal, safely extract the minor artifact, claim the distinct Perfect Jupiter departure slingshot to open Saturn, recover Saturn's artifact to lock Uranus, then recover Uranus's artifact, complete Orbit, and claim the Neptune Vector.
...
The retired Reach/Control/Recovery proving cards are not restored from prior campaigns. Generic thrust, sensors, escape, pressure, volatility, payout, and unrelated modules continue serving Flyby, Orbit, Mining, crew, and other systems but do not alter live launch survival.
...
Author a reusable progression beat by adding a versioned `ScenarioDefinition`, unique local step IDs, prerequisite edges, explicit goal/gate/next-step copy, typed `ScenarioActivityKind`, `ScenarioTransition`, retry policy, presentation mode, a typed `ScenarioEventKind`, and typed rewards. Reward kinds can grant unlock keys, destination route access, Support Drones, bay capacity, safely delivered Common/Rare/Exotic materials, or an explicit campaign milestone. A fixed mining activity references a versioned `MiningSiteDefinition`; a Flyby step uses `FlybyFinished` plus `requiredGrade`. Route gates are content data: a destination requires unlock keys, while a `RouteAccess` reward queues the matching physical route. `validateCampaignProgressionCatalog()` rejects duplicate IDs, cyclic prerequisites, unreachable key producers, invalid reward routes, dead actions, incomplete player guidance, unknown mining sites, invalid cocoons, and procedural factories whose template would instantiate by default. `CampaignNextStep` is the shared presentation consumed by all progression-facing screens.

Save version 16 is a strict fresh-start boundary. The loader accepts exactly version 16, clears every older, future, or malformed campaign payload, and immediately stores a fresh v16 campaign. Preferences remain intact. Stable hub states also write a last-known-good checkpoint; a failed runtime progression audit never overwrites the active campaign or checkpoint and exposes explicit checkpoint restoration or New Campaign rather than silently repairing state.
...
- `src/core/SaveData.*` persists only the current version-16 campaign and Transport-run state. Every non-v16 payload is rejected before any field can restore.
...
The campaign save format is versioned and line-based. Version 16 is the current and only accepted schema. It persists scenario instances, typed route transit, explicit story acknowledgement state, permanent Survey Array, Bore System, and Rig Fuel Loop ranks, Expedition XP, queued choices, generated offers, temporary Rig ranks, Drone ranks, slot grafts, selected synergies, and active Mining runtime alongside permanent campaign state. Non-v16 or malformed campaigns are intentionally not migrated; they are cleared and immediately replaced with a fresh v16 save. Validated stable hubs additionally write a separate v16 checkpoint. `SaveSchema.h` remains authoritative for current field names and defaults.
