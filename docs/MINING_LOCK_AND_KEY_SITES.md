...
- Gate-associated cells and enemies serialize with the active arena. New authored and procedural sites are held by `ScenarioInstance` plus `MiningSiteDefinition`; active v23 runs resume their own persisted site state without reconstructing pre-scenario provenance.
...
Runtime progress stores the authored cocoon ID/version, protected objective, active layer, per-layer reveal/completion state, and each cell's layer tag for the active and cached depth layers. `MiningSiteProgress` stores reusable site identity. Reload never reconstructs a custom cocoon from player-facing labels or assumes a fixed two-layer shape.
