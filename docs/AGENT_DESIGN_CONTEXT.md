...
Source PDFs are preserved under `docs/reference/source-pdfs/`. Markdown extracts exist so agents can read the design without PDF tooling.
...
- Surface loop: landing opens survey, mining, deeper excavation, payload extraction, and temporary field upgrades.
- Mining mini-game: direct drone control, destructible chunked terrain, fog of war, scanner pulses, drill friction, ore pockets, artifacts, oxygen, cargo, and extraction risk.
...
Drone systems should start as environmental mining support in the solar system. Combat drones are future-facing and should stay locked until the game leaves Earth's solar system.
