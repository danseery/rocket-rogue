...
- Direct Launch upgrades: Fuel Tanks, Flight Controls, Engine Cooling, and Hull Plating each have three predictable ranks at 22 credits per rank. Lesson results reveal only the newly taught upgrade; Ship Details lists the current and next numerical effect without rarity, rerolls, or randomized launch cards. Installed ranks survive replacement ships.
...
- The first Moon survey runs at fixed 60% throttle with only Fuel visible. Turn around at the halfway fuel light: five units out and five back completes the survey, pays 22 credits, and unlocks Fuel Tanks I. Moon transit costs 10 fuel plus a visible 5-unit insertion reserve, so the base 10-unit tank cannot arrive.
- Flight-control calibration then enables the corridor, persistent throttle, and left/right steering. Direction is never reversed; the uncalibrated ship carries momentum, oversteers, and receives seeded lateral kicks only when throttle rises. Flight Controls remove that chaos rank by rank.
...
- Every Launch upgrade rank costs 22 credits. Early qualified returns pay exactly 22, and the tutorial starts with no free spending credits or recovery-floor exploit.
