...
- Jupiter, Saturn, Uranus, and Neptune remain independent destinations with stable history and map states. The retired `outer_planets` id stays reserved and must not be reused; current version-14 saves use the individual destination IDs.
...
- Chapter 4: Breakthrough - Io Hazard Support Drone commissioning, a one-layer thermal-sealed artifact, the Perfect Jupiter-to-Saturn slingshot, Saturn artifact recovery to unlock Uranus, and the authored Uranus departure. `Signal Beyond Neptune` introduces a repeating carrier signal without naming its source. The player banks 8 Flight Data (`Landing +1`; Good or Perfect `Pass Through` completes the vector; Orbit alone grants none), claims `Lock Neptune Course`, and flies the saved Uranus -> Neptune leg. Successful Neptune arrival triggers the full-screen Straylight discovery beyond Neptune.
