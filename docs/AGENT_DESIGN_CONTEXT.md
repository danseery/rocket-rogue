...
- Save version 9 retains version 8's rig/operator, vector-gravity, loose-chunk, disabled-rig, artifact-tether, and purchased duplicate Support Drone state while adding saved scenario instances, mining-site provenance, and generic cocoon/protected-objective layer state. Older saves restore seated, migrate Support Drones to `ControlledActor`, de-duplicate repeated pre-v8 loadout IDs once, and synthesize v9 scenario/cocoon state without replaying rewards.
...
- Register static art in the required shared texture manifest. The runtime texture count is 36, including `JetpackCapybara` and the unlabeled `PoiGuidanceArrow`; missing artwork or registration is a validation error, not a fallback-sprite case.
