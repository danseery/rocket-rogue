...
- `approach` starts at 60 m with a 4 m/s descent; `correction` starts at 35 m with upward and lateral drift; `inverted` points the nose down; `fast` retains a dangerous 24 m/s descent; `touchdown` starts 2 m above the pad; `climb` approaches the 46 m exit boundary from below. No inputs are automated.
...
Touchdown shake and controller rumble scale in duration with impact speed. The full effect is reserved for severe impacts; reduced motion removes the jolt. The two-second arrival celebration and immediate Deploy Surface Team action remain independent of camera movement, and reduced-motion settings remove the jolt.
