# Surface Arrival and Presentation
...
- `approach` starts at 60 m with a 4 m/s descent; `correction` starts at 35 m with upward and lateral drift; `inverted` points the nose down; `fast` retains a dangerous 24 m/s descent; `touchdown` starts 2 m above the pad; `climb` approaches the 60 m exit boundary from below. No inputs are automated.
...
An authorized inward crossing of the marked 60-degree descent gate at radius 0.40 commits to Landing. The fixed basis projects orbital momentum into local horizontal and vertical velocity using a constant 12:1 conversion, with no speed clamp. Entry altitude is 60 m. A 1.25-second handoff blends the camera into the exact prepared site. Gravity, controls, prediction and collision then use the local frame. The surface origin stays fixed while the ship moves sideways or descends through cached underground layers. The explicit Land command from surveyed orbital inspection first stops and aligns the ship; manual gate entry has no such reset. Climb through 60 m with at least 2 m/s upward velocity to return to Orbit using the inverse momentum conversion. The gate must rearm outside radius 0.55 before another entry.
...
In the committed local Landing activity, collision damage is `max(0, impactSpeed - 8) * 5` hull points. Impact speed is inward contact-point velocity, including rotation; parallel motion is not a head-on impact. Terrain contacts are aggregated into one episode. The first contact with real support, adjacent rig clearance, and tilt within 30 degrees of upright commits touchdown immediately, including a small upward drift reported by collision integration. The departure support latch prevents an intentional ascent from relanding. Other surviving contacts rebound at 15% inward speed, retain 85% tangential speed, and halve rotation. Only hull depletion destroys the ship during terrain collision; there is no separate touchdown speed cap. Planet-body contact in Fly or Orbit is always fatal, irrespective of speed or remaining hull, with no rebound. The actual movement mode, not camera blend, selects this rule; descent-gate authorization still applies. Asteroid damage remains unchanged.

Touchdown uses a short, restrained camera jolt that settles within 0.36 seconds. Hard landings increase it modestly. The two-second arrival celebration and immediate Deploy Surface Team action remain independent of camera movement, and reduced-motion settings remove the jolt.

Additional collision presets: `impact` gives an approximately 18 m/s contact with 85 hull; `fatal` repeats it with 40 hull; `scrape` starts tilted with sideways motion; `rest` checks quiet settling; `bodyimpact` hits the planet outside the landing gate. Impact speed, damage, and hull before/after are shown in the result modal and Flight Report, not the side panel. These display records are session-only; saved hull state is unchanged.

Handling and presentation acceptance require direct play checks in addition to automated state checks.
